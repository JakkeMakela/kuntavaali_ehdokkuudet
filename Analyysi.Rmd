...
plotIkaEffect(kvtilasto, ymin=30, ymax = 80,
              caption = "KV, Listan keski-ikä vs listan kokeneisuus") %>% 
  print()
...
```{r}
lm2 <- lmer(100*vanhojen_osuus ~ ika.ka +  1|puolue  , data = kvtilasto)
...
```{r}
lm.vanh <- lm(100*vanhojen_osuus ~ listan_pituus:puolue , data = kvtilasto)
...
lm.ika <- lm(ika.ka ~ puolue + kuntavaali, data = kokotilasto) 
